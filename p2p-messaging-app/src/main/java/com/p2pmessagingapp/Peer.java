...
public class Peer {
    ...
    private static SSLSocket sslSocket; // SSL socket for secure communication
    private static String[] values; // Array to hold user input values (ID and port)
    private static PeerServer serverThread; // Thread for the peer server
    private static BufferedReader bufferedReader; // Buffered reader for user input
    ...
    public static void main(String[] args) throws Exception {
        ...
        addShutdownHook(); // Triggered by pressing 'Control + C'
        ...
        while (true) {
            ...
            boolean verificationStatus = fileAndPortVerification(values); // Verify input values
    ...
    public Peer(String address, int port) throws Exception {
        createUserAtributtes(address, port); // Create user attributes for a new peer
    }
     ...
     */
    public static void askForcommunication(BufferedReader bufferedReader, String id, PeerServer serverThread)
            throws Exception {
        ...
        while (true) {
            ...
            if (otherPeerID.equals("%% exit")) {
                deleteClientFile(values[0]); // Clean up client file
                deletePortLine(); // Clean up port line
                Peer.main(values);
            }
     ...
     */
    public static void communicate(BufferedReader bufferedReader, String id, PeerServer serverThread, User receiver,
            SSLSocket sslSocket) {
        try {
            ...
            OUTER: while (true) {
                ...
                switch (content) {
                    ...
                    case "%% change":
                        ...
                        askForcommunication(bufferedReader, id, serverThread); // Prompt for a new peer
                        break;
                    default:
            ...
            Peer.main(values); // Return to the main peer interface
        } catch (Exception e) {
        }
    }
     ...
     */
    private static void createSSLSocket(String address, int port) {
        try {
    ...
    private static void addShutdownHook() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            try {
                ...
                deletePortLine(); // Passes the current client's port
            } catch (IOException e) {
            }
        }));
    }
     ...
     * 
     * @param values The array containing the ID and port values to be verified.
